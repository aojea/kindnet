...
#   else \
#       CGO_ENABLED=1 GOARCH=$TARGETARCH go build -o /go/bin/cni-kindnet . ;\
# #       apt-get update && apt-get -y install gcc-x86_64-linux-gnu g++-x86_64-linux-gnu ;\
# #       CC=x86_64-linux-gnu-gcc CGO_ENABLED=1 GOARCH=$TARGETARCH go build -o /go/bin/cni-kindnet . ;\
#   fi
